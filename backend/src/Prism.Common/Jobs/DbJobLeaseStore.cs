...
public sealed class DbJobLeaseStore : IJobLeaseStore
{
    ...
    public async Task<DurableJob?> ClaimNextAsync(IReadOnlyList<string> jobTypes, CancellationToken ct)
    {
        ...
        // Exactly-once comes from doing the select and the status change in one statement:
        // read-then-update would let two workers observe the same queued row and both take it.
        // SKIP LOCKED is a throughput property, not a correctness one - plain FOR UPDATE is
        // still safe here, because PostgreSQL re-checks the predicate after the lock releases
        // and the waiting worker then sees the row is no longer queued. SKIP LOCKED simply
        // lets that worker step over the row instead of blocking on it. Verified by mutation:
        // removing SKIP LOCKED does not break the concurrency test, and should not.
        const string sql = """
            UPDATE jobs
